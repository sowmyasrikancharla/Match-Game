...
class MatchGame extends Component {
  ...
  updateScore = id => {
    const {activeImageId, activeImageUrl, isPlaying, score} = this.state
    if (isPlaying === true && activeImageId === id) {
      const imgObject =
        imagesList[Math.floor(Math.random() * imagesList.length)]
      this.setState({
        score: score + 1,
        activeImageUrl: imgObject.imageUrl,
        activeImageId: imgObject.id,
      })
    } else {
  ...
  display = () => {
    ...
    if (isPlaying === true) {
      return (
        <div>
          <ul>
            {' '}
            <FullImage imgUrl={activeImageUrl} key={activeImageUrl} />
          </ul>
          <div>
            <ul className="flex-con">
    ...
    return <ScoreCard score={score} onReset={this.onReset} />
  }

  render() {
    const {isPlaying, score, time} = this.state
    console.log(imagesList[Math.floor(Math.random() * imagesList.length)])
    return (
      <div className="main-con">
        <Navbar score={score} time={time} />
        {this.display({imagesList})}
      </div>
    )
  }
}
